...
public class SendAudioMessage : UnreliableMessage
{
    ...
    public byte[] Message;
    public SendAudioMessage(byte[] message)
    {
        Message = message;
    }
    ...
    protected override int GetMessagePayloadLength()
    {
        return MessageFields.LengthByteArray(Message);
    }
    ...
    protected override Message? InterpretMessagePayload(ReadOnlySpan<byte> payload)
    {
        ...
        if(message is null) return null;
        return new SendAudioMessage(message);
    }
}
...
public class AudioMessage : UnreliableMessage
{
    ...
    public ushort ClientId;
    public byte[] Message;
    public AudioMessage(ushort clientId, byte[] message)
    {
        ClientId = clientId;
        Message = message;
    }
    ...
    protected override int GetMessagePayloadLength()
    {
        return MessageFields.LengthUInt16() + MessageFields.LengthByteArray(Message);
    }
    ...
    protected override Message? InterpretMessagePayload(ReadOnlySpan<byte> payload)
    {
        ...
        byte[]? message = MessageFields.ReadByteArray(ref payload);
        if(message is null) return null;
        return new AudioMessage(clientId.Value, message);
    }
}